...
def r_squared(y, estimated):
    ...
    ss_res = sum((np.array(y)-np.array(estimated)))**2
    
    #calculate sum of squares (proportional to the variance of data)
    ss_tot = sum([(yi-mean)**2 for yi in y])  
    
    return 1 - (ss_res/float(ss_tot))
    '''
    return 1 - (sum((np.array(y)-np.array(estimated))**2) / float(sum([(yi-np.mean(y))**2 for yi in y])))
...
def evaluate_models_on_training(x, y, models):
    ...
    for model in models:
        y_models.append(np.polyval(model, x))
    if len(models[0] == 2):
        y_model = np.polyval(models[0], x)
    else:
        return
    ...
    for y_model in y_models:
        pylab.clf()
        pylab.plot(x,y, 'b.')
        pylab.plot(x,y_model, 'r-')
        pylab.ylabel("Temperature")
        pylab.xlabel("Year")
        pylab.title("Regression Model Degree {}\nR^2={}".format(len(model)-1, r_squared(y, y_model)))
